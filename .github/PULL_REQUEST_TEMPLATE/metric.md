...
`source` block, normally using `{url}` or `{metric_source}`, plus reusable env/file secret
...
- [ ] The reusable source uses `{url}` or `{metric_source}` and env/file secret references where required.
