...
- Update Fastify WebSocket support.

## [0.8.1] - 2026-09-21

### Fixed

- Authenticate Jellyfin metrics with the Jellyfin 12 authorization header.

## [0.8.0] - 2026-09-21

### Added

- Add Jellyfin library metrics for active streams and media counts.

### Changed

- Update Fastify, js-yaml, Framer Motion, Astro React, Fallow, and documentation Astro dependencies.

## [0.7.4] - 2026-09-14

### Fixed

- Prevent service icon glimmers from starting on metric and information controls while allowing active glimmers to finish.

## [0.7.3] - 2026-09-13

### Fixed

- Show Docker health status in service badges when available.

## [0.7.2] - 2026-09-12

### Fixed

- Use selfh.st and Dashboard Icons variants only when the base icon lacks contrast.
...
- Add opt-in Homepage Docker-label fallbacks with `HOMEPAGE_LABEL_FALLBACK=true`.
...
- Start service icon glimmers when a touch tap begins, before navigation changes the page.

## [0.6.2] - 2026-09-02

### Fixed

- Dismiss the uptime graph tooltip when its selected bar is tapped again on touch devices.

## [0.6.1] - 2026-09-02

### Changed

- Refactor dashboard, metrics, configuration, Docker, and realtime internals to improve maintainability without changing behavior.

## [0.6.0] - 2026-09-02

### Added

- Add dashboard translations for Arabic, Chinese, Dutch, French, German, Italian, Japanese, Korean, Polish, Portuguese (Brazil), Russian, Spanish, Turkish, and Ukrainian, with Arabic right-to-left layout support.
- Add localized labels for bundled library metrics.

### Changed

- Update dashboard, documentation, and workflow dependencies.

## [0.5.1] - 2026-09-01

### Fixed

- Keep uptime metrics loading until their first observation is available.
...
- Reduce masonry layout measurement work during initial dashboard rendering.
...
- Use a pointer cursor for card description buttons.
...
- Dismiss selected uptime heartbeat tooltips when their bucket loses focus.
...
- Add a Gatus uptime metric configured with raw `group` and `name` inputs.
...
- Wait for an uptime observation before displaying a new heartbeat period.
...
- Move the About dialog trigger to the dashboard header.

### Removed

- Remove the no-op `dashmark.metric_providers` Docker label.

## [0.3.5] - 2026-08-27

### Changed

- **Breaking:** replace `service.metrics.catalog` and `service.metrics.local` with `service.metrics.entries`. Use `provider/metric` keys for library metrics and plain keys for custom metrics. Rename `service.metrics.source_url` to `service.metrics.api_url`, `dashmark.metrics_url` to `dashmark.api_url`, and `{metrics_url}` to `{api_url}`.

## [0.3.4] - 2026-08-26

### Fixed

- Parse fractional seconds in Authentik session expiry timestamps for the logged-in users metric.
