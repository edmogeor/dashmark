...
`source` block, normally using `{url}` or `{api_url}`, plus reusable env/file secret
...
- [ ] The reusable source uses `{url}` or `{api_url}` and env/file secret references where required.
