...
function chartData(detail: MetricDetail): ChartPoint[] {
  const data = downsampleChartData(
    metricData(detail.history, detail.series),
    detail.series,
  )
  return data.map((point, index, points) => {
    ...
    for (const series of detail.series) {
      ...
      if (index === points.length - 1 && typeof value === 'number')
        labels[`${series.key}Label`] = detail.formatValue(value)
    }
    return labels
  })
}
      ...
      content={({ active, label, payload }) => {
        const values =
          payload?.flatMap((item) => {
            const series = detail.series.find(
              (candidate) => candidate.key === item.dataKey,
            )
            const value = Number(item.value)
            return series && Number.isFinite(value)
              ? [[series, value] as const]
              : []
          }) ?? []
        ...
        return (
          <div className="dashmark-metric-chart-tooltip grid gap-1 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl">
            <span className="dashmark-metric-chart-tooltip-time text-muted-foreground">
              {formatExactTimestamp(label)}
            </span>
            {values.map(([series, value]) => (
              <div
                key={series.key}
                className="dashmark-metric-chart-tooltip-value flex items-center justify-between gap-4 font-mono font-medium tabular-nums"
                data-series-key={series.key}
              >
                {detail.series.length > 1 && (
                  <span className="dashmark-metric-chart-tooltip-label text-muted-foreground">
                    {series.label}
                  </span>
                )}
                <span className="dashmark-metric-chart-tooltip-number">
                  {(detail.formatTooltipValue ?? detail.formatValue)(value)}
                </span>
              </div>
            ))}
          </div>
  ...
  return (
    <LabelList
      ...
      content={(props) => {
        ...
        const offset = endLabelOffset(
          series.key,
          y,
          labels,
          domain,
          props.viewBox as { y?: number; height?: number } | undefined,
        )
        return (
          <g
            className="dashmark-metric-chart-end-label"
            transform={`translate(${x - width - 4} ${y - 12 + offset})`}
          >
...
function MetricSeriesLine({
  ...
  return chart === 'area' ? (
    <Area
      className="dashmark-metric-chart-series"
      {...props}
...
function MetricChart({ detail }: { detail: MetricDetail }) {
  ...
  const endLabels = detail.series.flatMap((series) =>
    typeof data.at(-1)?.[series.key] === 'number'
      ? [{ key: series.key, value: data.at(-1)![series.key] as number }]
      : [],
  )
  ...
  const axisFormatter = (value: unknown) =>
    Number.isFinite(Number(value))
      ? (detail.formatAxisValue ?? detail.formatValue)(Number(value))
      : ''
  return (
    <div className="dashmark-metric-dialog-body flex h-80 w-full flex-col">
      <div className="min-h-0 flex-1">
        <ChartContainer
          config={tickerConfig}
          className="dashmark-metric-chart h-full w-full aspect-auto"
          aria-label={`${detail.label} chart`}
        >
          <Chart
            className="dashmark-metric-chart-svg"
            data={data}
          ...
          >
            ...
            <XAxis
              ...
              ticks={Array.from(
                { length: 4 },
                (_, index) => start + ((end - start) * index) / 3,
              )}
            />
            <YAxis
      ...
      {detail.series.length > 1 && (
        <div className="dashmark-metric-chart-legend flex shrink-0 justify-center gap-4 pt-2 text-xs leading-none font-medium text-muted-foreground normal-case">
          {detail.series.map((series) => (
            <span
              key={series.key}
              className="dashmark-metric-chart-legend-item flex items-center gap-1.5"
              data-series-key={series.key}
            >
              <span
                className="dashmark-metric-chart-legend-swatch h-2 w-2 shrink-0 rounded-full"
...
export const MetricDetailDialog = memo(function MetricDetailDialog({
  detail,
  onOpen,
  onOpenChange,
}: {
          ...
          <>
            <DialogHeader className="dashmark-metric-dialog-header !flex-row !items-center !justify-between !space-y-0">
              ...
              <DialogDescription className="sr-only">
                Live {currentDetail.label.toLowerCase()} details
              </DialogDescription>
            </DialogHeader>
