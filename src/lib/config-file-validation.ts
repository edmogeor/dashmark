...
import {
  CUSTOM_METRIC_BADGE_COLORS,
  CUSTOM_METRIC_CHARTS,
  CUSTOM_METRIC_REDUCTIONS,
  CUSTOM_METRIC_UNITS,
  type CustomMetricReduction,
  type MetricOverride,
  type MetricHttpAuth,
  type MetricHttpRequest,
  type MetricJsonValue,
  type MetricParameter,
  type MetricPagination,
  type MetricRequestValue,
  type MetricSecretReference,
  type MetricSourceOverride,
  type MetricTokenExtractor,
  type MetricUrlTransform,
  type MetricValueReference,
  type MetricTransform,
  type CustomMetricChart,
  type MetricUnit,
  type PrometheusMetricExtractor,
  type JqMetricExtractor,
  type ForEachMetric,
  type SocketIoArgument,
  type SocketIoMetricSource,
  type NumericMetricOverride,
  type ServiceMetricOverrides,
  type ServiceMetrics,
...
const SERVICE_FIELDS = new Set(['title', 'description', 'url', 'icon', 'category', 'host', 'order', 'hidden', 'show_status', 'access', 'search_aliases', 'metrics'])
...
function parseReduction(value: unknown): CustomMetricReduction | undefined {
  return typeof value === 'string' && isOneOf(CUSTOM_METRIC_REDUCTIONS, value) ? value : undefined
}

function parseUnit(value: unknown): MetricUnit | undefined {
  if (typeof value === 'string' && isOneOf(CUSTOM_METRIC_UNITS, value)) return value
...
function parseChart(value: unknown): CustomMetricChart | undefined {
  return typeof value === 'string' && isOneOf(CUSTOM_METRIC_CHARTS, value) ? value : undefined
}

function parseMetricStateColor(value: unknown): CustomMetricStateColor | undefined {
  return typeof value === 'string' && isOneOf(CUSTOM_METRIC_BADGE_COLORS, value) ? value : undefined
}
...
function parseMetricParameters(value: unknown, transforms?: unknown): Record<string, MetricParameter> | undefined {
  ...
  for (const [name, parameter] of Object.entries(value)) {
    if (
      ...
      !isRecord(parameter) ||
      Object.keys(parameter).some((key) => key !== 'label' && key !== 'type' && key !== 'transform') ||
      typeof parameter.label !== 'string' ||
      !parameter.label ||
      (parameter.type !== 'url_component' && parameter.type !== 'json_value')
    )
      return undefined
    const transform =
      parameter.transform === undefined
        ? undefined
...
function parsePrometheusExtractor(value: unknown): PrometheusMetricExtractor | undefined {
  ...
  if (
    ...
    (value.labels !== undefined && !isRecord(value.labels)) ||
    (value.reduce !== undefined && !reduce) ||
    (value.value_label !== undefined && !valueLabel)
  )
    return undefined
  return {
    name,
    ...(value.labels === undefined ? {} : { labels }),
...
function parseMetricTransform(value: unknown): MetricTransform | undefined {
  ...
  const add = value.add
  if (
    (multiply !== undefined && (typeof multiply !== 'number' || !Number.isFinite(multiply))) ||
    (add !== undefined && (typeof add !== 'number' || !Number.isFinite(add))) ||
    (multiply === undefined && add === undefined)
  )
    return undefined
  return {
    ...(multiply === undefined ? {} : { multiply }),
    ...(add === undefined ? {} : { add })
  }
}

function parseMetricHeaders(source: Record<string, unknown>): {
  ...
  const references = (value: unknown, kind: string): { values?: Record<string, MetricRequestValue>; error?: string } => {
    ...
    for (const [name, reference] of Object.entries(value)) {
      ...
      if (value === undefined || !name)
        return {
          error: `${kind} must use valid names and scalar values, secret, or token references`
        }
      values[name] = value
    }
    return { values }
  ...
  if (headers.error || (headers.values && !Object.keys(headers.values).every((header) => /^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/.test(header))))
    return {
      error: headers.error ?? 'headers must use valid names and scalar values, secret, or token references'
    }
  const query = references(source.query, 'query')
  if (query.error) return { error: query.error }
  return {
    ...(headers.values && Object.keys(headers.values).length > 0 ? { headers: headers.values } : {}),
    ...(query.values && Object.keys(query.values).length > 0 ? { query: query.values } : {})
  }
}
...
function parseValueReference(value: unknown): MetricValueReference | undefined {
  ...
  if (
    ...
    !/^[a-z][a-z0-9_-]*$/.test(value.token) ||
    Object.keys(value).some((key) => key !== 'token' && key !== 'prefix') ||
...
function parseJsonValue(value: unknown): MetricJsonValue | undefined {
  ...
  if (!isRecord(value)) return undefined
  const entries: Record<string, MetricJsonValue> = {}
...
function parseMetricRequestBody(body: unknown, path: string, kind: 'form'): { values?: Record<string, MetricRequestValue>; error?: string }
function parseMetricRequestBody(body: unknown, path: string, kind: 'json'): { values?: Record<string, MetricJsonValue>; error?: string }
function parseMetricRequestBody(body: unknown, path: string, kind: 'form' | 'json'): { values?: Record<string, MetricRequestValue> | Record<string, MetricJsonValue>; error?: string } {
  ...
  const values: Record<string, MetricRequestValue | MetricJsonValue> = {}
...
function parseMetricRequest(value: unknown, path: string): { request?: MetricHttpRequest; error?: string } {
  ...
  if (!url || !isMetricUrl(url))
    return {
      error: `${path}.url must use HTTP or HTTPS, or begin with {url} or {metric_source}`
    }
...
function parseSocketIoArguments(value: unknown, path: string): { args?: SocketIoArgument[]; error?: string } {
  ...
  if (!Array.isArray(value))
    return {
      error: `${path} must be a list of strings, numbers, booleans, or secret references`
    }
...
function parseSocketIoEvent(value: unknown, path: string, requireArgs = false): { event?: { event: string; args?: SocketIoArgument[] }; error?: string } {
  if (!isRecord(value) || typeof value.event !== 'string' || !value.event || Object.keys(value).some((key) => !['event', 'args'].includes(key))) {
    return {
      error: `${path} must define an event name and optional arguments`
    }
  }
...
function parseSocketIoSource(value: unknown): {
  ...
  if (Object.keys(value).some((key) => !['url', 'transport', 'headers', 'auth', 'socketio'].includes(key)))
    return {
      error: 'Socket.IO sources only support url, transport, headers, auth, and socketio'
    }
...
function parseSharedMetricSources(value: unknown): Record<string, Record<string, unknown>> | undefined {
  ...
  for (const [name, source] of Object.entries(value)) {
    ...
    const normalized = normalizeMetricSource({
      ...sourceSettings,
      url: baseUrl
    })
    if (normalized.error || !isRecord(normalized.source)) invalid(`shared_metric_sources.${name}`, normalized.error ?? 'a valid source')
    const request = parseMetricRequest(withoutAuth(normalized.source), `shared_metric_sources.${name}`)
    ...
    sources[name] = request.request ? { ...request.request, ...(auth.auth ? { auth: auth.auth } : {}) } : {}
  }
  return sources
}

function withoutAuth(source: Record<string, unknown>): Record<string, unknown> {
  const { auth: _auth, ...request } = source
  return request
}

function metricDefinitionFields(
  value: unknown,
  partial: boolean
): {
  fields?: {
  ...
  if (isRecord(metricValue) && Object.keys(metricValue).some((key) => !['kind', 'unit', 'rate', 'transform', 'default_color', 'colors', 'labels'].includes(key)))
    return { error: 'value contains an unknown configuration key' }
...
export function normalizeMetricDefinition(
  value: unknown,
  providerCharts?: unknown,
  partial = false,
  sharedSources?: Record<string, Record<string, unknown>>,
  providerTransforms?: unknown
): { definition?: Record<string, unknown>; error?: string } {
  ...
  if (isRecord(value) && value.parameters !== undefined && !parameters)
    return {
      error: 'parameters must define named URL-component parameters and optional provider transforms'
    }
  ...
  if (chartName !== undefined && !chartStyle && !parseChart(chartName))
    return {
      error: 'display.chart must be a chart style or configured chart group'
    }
...
function buildCustomMetric(metric: Record<string, unknown>, fields: ParsedCustomMetricFields, source: MetricSourceOverride): { metric?: MetricOverride; error?: string } {
  ...
  const common = { label, ...(parameters ? { parameters } : {}), ...(pagination ? { pagination } : {}), source }
  if (valueType === 'uptime') {
  ...
  if (valueType === 'string') {
    if (text) return { metric: { ...common, valueType, text: true } }
    if (jq) return { metric: { ...common, valueType, jq } }
    return prometheus ? { metric: { ...common, valueType, prometheus } } : { error: 'string metrics require an extractor' }
  }
  if (valueType === 'state') {
    if (!color) return { error: 'state metrics require a color' }
    const state = { ...common, valueType: 'state' as const, color, ...(stateColors ? { stateColors } : {}), ...(stateLabels ? { stateLabels } : {}) }
    if (text) return { metric: { ...state, text: true } }
    if (jq) return { metric: { ...state, jq } }
    return prometheus ? { metric: { ...state, prometheus } } : { error: 'state metrics require an extractor' }
  }
  if (!unit || !chart) return { error: 'numeric metrics require a valid unit and chart' }
  const numeric = {
    ...common,
  ...
  if (forEach) return { metric: { ...numeric, forEach } }
  if (text) return { metric: { ...numeric, text: true } }
  if (jq) return { metric: { ...numeric, jq } }
  return prometheus ? { metric: { ...numeric, prometheus } } : { error: 'numeric metrics require an extractor' }
}
...
function parseCustomMetricRequestSource(
  ...
  if (transport === 'socketio' && auth?.optional)
    return {
      error: 'optional authentication is only supported for HTTP metrics'
    }
  return {
    source:
      transport === 'socketio'
        ? {
            url,
            transport: 'socketio',
            ...(socketHeaders.headers ? { headers: socketHeaders.headers } : {}),
            ...(auth ? { auth } : {}),
...
function parseMetricOverrides(
  ...
  partial = false,
  sharedSources?: Record<string, Record<string, unknown>>
): { metrics?: ServiceMetricOverrides; errors?: Record<string, string> } {
  ...
  for (const [key, configuredMetric] of Object.entries(value)) {
    ...
    const parsed = normalized.definition ? parseCustomMetric(key, normalized.definition, catalog) : { error: normalized.error }
...
export function parseMetricCatalog(catalog: Record<string, Record<string, unknown>>): ServiceMetricOverrides {
  return Object.fromEntries(
    Object.entries(catalog).flatMap(([key, definition]) => {
...
function parseDuration(value: unknown): number | undefined {
  ...
  const scales: Record<string, number> = { ms: 1, s: 1_000, m: 60_000, h: 3_600_000, d: 86_400_000 }
  const unit = match[2]
  if (!isOneOf(Object.keys(scales), unit)) return undefined
...
function parseBuiltInMetricEntry(name: string, definition: unknown, entryPath: string, parts: MetricEntryParts): boolean {
  if (!isOneOf(RESOURCE_STATS, name)) return false
...
function parseCatalogMetricEntry(name: string, definition: unknown, entryPath: string, catalog: Record<string, Record<string, unknown>>, parts: MetricEntryParts): boolean {
  ...
  if (configured.inputs !== undefined) {
    ...
    configuredInputs = {}
    for (const [input, value] of Object.entries(configured.inputs)) {
      if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') configuredInputs[input] = value
    }
  }
...
function parseServiceMetrics(value: unknown, path: string, catalog: Record<string, Record<string, unknown>>, sharedSources?: Record<string, Record<string, unknown>>): ServiceMetrics | undefined {
  if (value === undefined) return undefined
...
function parseService(value: unknown, path: string, catalog: Record<string, Record<string, unknown>>, sharedSources?: Record<string, Record<string, unknown>>): ServiceOverrides {
  if (!isRecord(value)) invalid(path, 'a mapping')
  ...
  return {
    ...
    metrics: parseServiceMetrics(value.metrics, `${path}.metrics`, catalog, sharedSources)
  }
}
...
function parseSettings(value: unknown): YamlSettings {
  ...
  for (const key of [
    ...
    validateString(value[key], `settings.${key}`)
  for (const key of [
    'enable_access_control',
    'show_search',
    'show_status',
...
export function parseYamlConfig(value: unknown, catalog: Record<string, Record<string, unknown>>): YamlConfig {
  ...
  for (const [name, service] of Object.entries(value)) {
    ...
    services[name] = parseService(service, name, catalog, sharedMetricSources)
  }

  return {
    settings: parseSettings(value.settings),
    ...(sharedMetricSources ? { sharedMetricSources } : {}),
    services
  }
}
