...
The retired YAML keys `metrics` list, `metric_providers`, `metrics_url`, `metric_parameters`, `metrics_poll_interval`, `metrics_history_period`, `metrics_access`, `custom_metrics`, and `source_url` have no aliases. The retired Docker label `dashmark.metrics_url` and metric template `{metrics_url}` also have no aliases. Docker labels are still accepted separately; library selection no longer requires a provider label gate.
