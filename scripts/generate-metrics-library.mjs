...
if (process.argv.includes('--check')) {
  if (!fs.existsSync(libraryPath) || fs.readFileSync(libraryPath, 'utf8') !== output) {
    console.error('metrics/LIBRARY.md is out of date. Run npm run generate:metrics-library.')
    process.exitCode = 1
  }
} else {
  fs.writeFileSync(libraryPath, output)
  if (staged) execFileSync('git', ['add', 'metrics/LIBRARY.md'])
}
