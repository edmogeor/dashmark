...
export async function getMetricsResponse(request: Request): Promise<Response> {
  ...
  const body: MetricsResponse = {
    ...
    customMetrics,
    uptimeMetrics: process.env.MOCK_AUTH === 'true' ? demoUptimeMetrics(cardId ?? '', Date.now()) : (metricUsage?.uptimeMetrics ?? []).filter((metric) => visible(metric.key)),
