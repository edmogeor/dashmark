...
function validateSocketIoSource(source) {
  ...
  if (typeof source.url !== 'string' || !sourceBase.test(source.url)) throw new Error('source.url must begin with {url} or {metric_source}')
...
function validateRequest(request, context, allowToken) {
  ...
  if (typeof value.url !== 'string' || !sourceBase.test(value.url)) throw new Error(`${context}.url must begin with {url} or {metric_source}`)
