...
The selfhst index and its precomputed contrast values are loaded from bundled `src/data/icons.json` and `src/data/icon-contrast.json` (generated by `npm run fetch-icons`, gitignored, copied into the image). When the icon index is unavailable, it is paged from the GitHub API.

Selfhst icons are analysed for luminance during the build. If an icon is dominated by very dark or very light colours, Dashmark stores a `contrast` hint and the client swaps to the matching `-light` or `-dark` monochrome variant whenever the active theme would otherwise make the icon hard to see. Custom file icons and arbitrary URLs are not analysed and always render as provided. The analysis downloads and processes the selfhst icon archive during `prebuild`, so contrast changes upstream are picked up on the next Dashmark build.
