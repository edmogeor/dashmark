...
export function uptimeBuckets(
  ...
  return Array.from({ length: bucketCount }, (_, index) => {
    ...
    const responseTimes = entries.flatMap((entry) =>
      entry.responseTimeMs === undefined ? [] : [entry.responseTimeMs],
    )
    return {
      start: bucketStart,
      end: bucketEnd,
...
export function formatUptimeBucketTime(timestamp: number): string {
  ...
  const calendarDate = new Intl.DateTimeFormat(undefined, { month: 'short', day: 'numeric' }).format(timestamp)
  const relativeDay = timestamp >= startOfDay
    ? 'Today'
    : timestamp >= yesterday
      ? 'Yesterday'
...
function bucketLabel(bucket: UptimeBucket): string {
  const counts = bucket.status === 'unknown'
...
const statusLabel: Record<UptimeBucketStatus, string> = {
  up: 'Up',
  down: 'Down',
  mixed: 'Partial',
  unknown: 'No data',
  ...
  return (
    <div className="grid gap-2">
      <div className="flex items-center justify-between gap-4 leading-none">
        ...
        <span className={cn('font-medium', statusTextClass[bucket.status])}>{statusLabel[bucket.status]}</span>
      </div>
...
function UptimeCell({
  ...
  const className = cn(
    'dashmark-uptime-heartbeat-cell min-w-0 rounded-sm',
