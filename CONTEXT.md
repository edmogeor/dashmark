...
3. Any other `icon` -> resolved inside `ICONS_DIR` as a path (subdirectories are allowed); a missing file returns a placeholder immediately (no further fallback).
4. No usable label -> fuzzy-match the container image name (and title's first word) against the selfhst index via `fuse.js`.
5. Fallback -> initials placeholder.
...
| `dashmark.icon` | URL, path inside `ICONS_DIR`, or `placeholder` (unset auto-matches) |
